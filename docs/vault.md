...
base, and it is the single source of truth — Bethink is a viewer and editor
...
That's it. Point Bethink at any folder of Markdown — a folder you already
...
2. **Builds an in-memory index** (an SQLite database held only in RAM) of
   ...
   search table.
3. **Starts a filesystem watcher** so changes made *outside* Bethink show up
   live.
...
Bethink and you have a tidy folder of Markdown, exactly as portable as before.
...
Because the files are always authoritative, Bethink can never drift out of
sync with what's on disk.

## Live filesystem sync

A recursive watcher folds in any change made to the vault folder while
Bethink is open — by git pulling a branch, by another editor, or by a sync
...
it from anywhere; Bethink simply keeps up.

## External edits and conflicts

When the watcher — or a [Stohr sync](stohr.md) — sees a page change on disk,
Bethink normally just re-loads it into the editor. But if you have **unsaved
...
The decision is always yours; Bethink never discards unsaved work on its own.
...
Bethink keeps this app-level metadata in a single small file,
...
- **New Vault…** — create a folder; if it's empty, Bethink seeds it with a
...
recent vaults is remembered between launches; on startup Bethink reopens the
vault you used last.

## First run

The very first time you launch Bethink — with no vaults yet — it creates a
default vault and seeds it with two pages, *Welcome to Bethink* and *Ideas*,
...
Bethink can connect to a [Stohr](https://github.com/wess/stohr) instance —
