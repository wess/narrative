...
//
// The reconcile pass is exported as `reconcilePaths` because it isn't only
// the watcher's: the Stohr sync also writes files straight to disk, and a
// platform whose recursive watch misses nested files (Linux) needs the same
// "these paths changed" pass run explicitly.
...
const rowByPath = (vault: OpenVault, path: string): NodeRow | undefined =>
  vault.db.query<NodeRow>("SELECT * FROM nodes WHERE path = ? LIMIT 1", path)[0];

const parentIdFor = (vault: OpenVault, relPath: string): number | null => {
  const dir = dirname(relPath);
  if (dir === "." || dir === "") return null;
  return rowByPath(vault, dir)?.id ?? null;
};
...
// Fold a batch of changed vault-relative paths into the in-memory index and
// notify the webview. Idempotent — a path whose content already matches the
// index is a no-op — so it's safe to run twice (watcher + sync) on the same
// paths.
export const reconcilePaths = async (vault: OpenVault, rels: string[]): Promise<void> => {
  ...
  for (const c of changes) {
    const row = rowByPath(vault, c.rel);
    if (!c.exists) {
    ...
    const fresh = rowByPath(vault, c.rel);
  ...
  for (const c of created) {
    if (rowByPath(vault, c.rel)) continue; // a rename already claimed it
    const now = new Date(c.mtime || Date.now()).toISOString();
    const parentId = parentIdFor(vault, c.rel);
    if (c.isDir) {
...
export const startWatcher = (vault: OpenVault): { close: () => void } => {
  ...
  const processBatch = async (): Promise<void> => {
    ...
    const rels = [...pending];
    pending.clear();
    await reconcilePaths(vault, rels);
  };
  ...
  const onEvent = (_type: string, filename: string | Buffer | null): void => {
    ...
    // Self-write filtering happens in `reconcilePaths`, where the file's
